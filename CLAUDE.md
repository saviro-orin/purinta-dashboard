...
- Keep the indexer public-RPC friendly: bounded `eth_getLogs` ranges, adaptive range splitting when RPCs reject larger requests, `PURINTA_INDEXER_DELAY_MS` between batches, `PURINTA_INDEXER_BLOCK_LAG` for reorg safety, and optional `ETHEREUM_RPC_FALLBACK_URL`.
